	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function details(id){
			  ...
			  details(id);  
		$(document).on("click","#quantity-up i",function(){
				  var plus=parseInt($("#sst").val());
				  $("#sst").val(plus+1);
		});
		$(document).on("click","#quantity-down i",function(){
				  if($("#sst").val()>1){
					$("#sst").val($("#sst").val()-1);   
				  }					   
		});
		$(document).on("click","#addCart",function(){
			size=$("option[class='option']:selected").val();		
			soluong=$("#sst").val();
			var ts=/[0-9]+$/;
			if((!ts.test(soluong.toString())) || (soluong<=0) ){
				alert("Amount Invalid.");
				return false;
			}
			else{
			addCart($(this).attr('title'),size,soluong);
			$('html body').append('<div id="nofi" style="background-color:rgba(85,85,85,0.93);width: 350px;height: 150px;position: fixed;top:20%;left: 40%;"><img src="img/success.jpg" alt="" width="50px" height="50px" style="border-radius: 50%; opacity: 1;margin-left: 150px;margin-top: 20px"><h2><p style="color:white;margin-left: 30px;margin-top: 20px">Add to cart success</p></h2></div>');
